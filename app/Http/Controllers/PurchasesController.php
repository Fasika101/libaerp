...
class PurchasesController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        // Global sums over the SAME filtered set (all pages) for the summary cards.
        $sums = (clone $Filtred)
            ->selectRaw('COALESCE(SUM(GrandTotal),0) as grand, COALESCE(SUM(paid_amount),0) as paid')
            ->reorder()
            ->first();
        $stats = [
            'count' => $totalRows,
            'total' => (float) ($sums->grand ?? 0),
            'paid' => (float) ($sums->paid ?? 0),
            'due' => (float) (($sums->grand ?? 0) - ($sums->paid ?? 0)),
        ];
            ...
            */
            ...
            if (PurchaseReturn::where('purchase_id', $id)->where('deleted_at', '=', null)->exists()) {
                ...
                foreach ($new_purchase_details as $key => $prod_detail) {
                    ...
                    // Absent on lines added during the edit (only stored details
                    // carry it), so default to 1 = "has a unit, process it" — the
                    // same default the batch/serial alignment below already uses.
                    // These three must agree or the aligned arrays desync from
                    // what actually got persisted.
                    if (($prod_detail['no_unit'] ?? 1) !== 0) {
    ...
    public function show(Request $request, $id)
    {
        ...
        // The detail page builds its PDF / email / SMS / delete calls from this,
