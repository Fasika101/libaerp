...
class SettingsController extends Controller
{
    ...
    // ----------------------- Module toggles (read-only for tenants) ----------------\\
    // Entitlements live on tenants.module_flags (set by Super Admin). Tenant UI
    // can view them; writes from tenant admins are rejected in SaaS mode.

    public function getModuleFlags(Request $request)
    {
        $user = $request->user('api');
        if ($user && $user->is_super_admin) {
            return response()->json(['module_flags' => null, 'readonly' => true], 200);
        }
        $tenant = \App\Support\TenantContext::get() ?: ($user?->tenant_id ? \App\Models\Tenant::find($user->tenant_id) : null);
        $flags = $tenant?->module_flags;
        if (is_string($flags)) {
            $flags = json_decode($flags, true);
        }

        return response()->json([
            'module_flags' => is_array($flags) ? $flags : null,
            'readonly' => true,
        ], 200);
    }
    ...
    {
        return response()->json([
            'message' => 'Module entitlements are managed by the platform Super Admin',
        ], 403);
    }
